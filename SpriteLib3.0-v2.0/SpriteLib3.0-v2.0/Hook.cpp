...
void Hook::Update()
{
	...
	if (attached)
	{
		...
		ECS::GetComponent<PhysicsBody>(MainEntities::MainPlayer()).ApplyForce(pullDirection * (10000)); // vector naturally gets smaller as player approaches the wall, making the pull start strong and end weaker.
