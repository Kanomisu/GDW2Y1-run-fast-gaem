...
void PhysicsPlayground::InitScene(float windowWidth, float windowHeight)
{
	...
	//Player entity
	{
		...
		tempPhsBody = PhysicsBody(entity, tempBody, float((tempSpr.GetHeight() - shrinkY) / 2.f), vec2(0.f, 0.f), false, PLAYER, ENVIRONMENT | ENEMY | OBJECTS | PICKUP | TRIGGER | HEXAGON, 3.f, 3.f);
		...
		ECS::GetComponent<Player>(entity).InitPlayer(&ECS::GetComponent<Transform>(entity), true, &ECS::GetComponent<PhysicsBody>(entity), &ECS::GetComponent<CanJump>(entity), m_physicsWorld);
		playerRef = entity;
	}
